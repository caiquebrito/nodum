...
async function init() {
  ...
  if (names.length === 0) { showEmpty(); return; }

  // Auto-select the project matching the current directory (passed by `nodum serve`).
  const hinted = new URLSearchParams(window.location.search).get('project');
  const initial = hinted && names.includes(hinted) ? hinted : names[0];
  loadProject(initial);
}
...
async function loadProject(name) {
  ...
  try {
    ...
    updateStats(graph);
    if (logsOpen) await loadLogsList();
  } catch {
    hideLoading();
...
function renderGraph(graph) {
  ...
  graph3d = ForceGraph3D({ antialias: true, alpha: false })(elem)
    ...
    .nodeOpacity(0.92)
    .nodeResolution(14)
    .linkColor(l  => l.relation === 'imports' ? '#1a4d9e' : l.relation === 'calls' ? '#c47f17' : '#1c2128')
    .linkOpacity(0.5)
    .linkWidth(l  => l.relation === 'imports' ? 0.8 : l.relation === 'calls' ? 0.5 : 0.3)
    .linkDirectionalArrowLength(l  => l.relation === 'imports' || l.relation === 'calls' ? 5  : 0)
...
async function loadLogsList() {
  ...
  if (dates.length === 0) {
    // Logs functionality disabled for now - not available via API
    // In future, add /api/projects/:name/logs endpoint
  }
